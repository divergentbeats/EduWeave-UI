    ...
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-slate-50 via-indigo-50/30 to-purple-50/30 dark:from-[#0b0b12] dark:via-[#0b0b12] dark:to-[#0b0b12] flex items-center justify-center p-4 transition-colors">
      ...
      <div className="bg-white dark:bg-[#12121b] text-gray-900 dark:text-indigo-100 p-8 rounded-2xl shadow-2xl w-full max-w-sm border border-gray-200 dark:border-gray-800 transition-colors">
        ...
        <div className="flex justify-center mb-4">
          <button onClick={() => setLoginType('student')} className={`px-4 py-2 rounded-l-lg ${loginType === 'student' ? 'bg-violet-600 text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>Student</button>
          <button onClick={() => setLoginType('admin')} className={`px-4 py-2 rounded-r-lg ${loginType === 'admin' ? 'bg-violet-600 text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>Admin</button>
        </div>

        <div className="space-y-6">
          <div>
            <label className="block text-gray-700 font-semibold mb-2">{loginType === 'student' ? 'USN' : 'Username'}</label>
            <input
              type="text"
              value={usn}
              ...
              placeholder={loginType === 'student' ? 'Enter your USN' : 'Enter your username'}
            />
          </div>
          <div>
            <label className="block text-gray-700 font-semibold mb-2">{loginType === 'student' ? 'Date of Birth' : 'Password'}</label>
            <input
